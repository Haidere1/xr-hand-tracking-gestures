...
AHanTracking::AHanTracking()
{
    ...
    static ConstructorHelpers::FObjectFinder<UStaticMesh> TipSphereMesh(TEXT("/Engine/BasicShapes/Sphere.Sphere"));
    if (TipSphereMesh.Succeeded())
    {
        TipSpheresL->SetStaticMesh(TipSphereMesh.Object);
        TipSpheresR->SetStaticMesh(TipSphereMesh.Object);
    }
...
void AHanTracking::DrawPinchRay(float DeltaSeconds)
{
    ...
    if (bL) {
        UpdateRaycastFromAim(EControllerHand::Left, ControllerDataLeft, bLPinch);
    ...
    if (bR) {
        ...
        UpdateTipSpheresFor(RP, EControllerHand::Right);
        UpdateRaycastFromAim(EControllerHand::Left, ControllerDataRight, bRPinch);
        ControllerDataRight.AimPosition = RP[(int32)EHandKeypoint::IndexTip];
        int32 IndexTip = (int32)EHandKeypoint::IndexTip;
...
void AHanTracking::UpdateRaycastFromAim(EControllerHand Hand, const FXRMotionControllerData& ControllerData, bool bIsPinching)
{
    ...
    // Ignore the actor itself during raycast

    bool bHit = GetWorld()->LineTraceSingleByChannel(
        HitResult,
        PalmPos, // Start the ray from the Palm Position
        HitResult.Location,  // End point calculated from the Aim Position
        ECC_Visibility, // Raycast against visibility (or any other appropriate channel)
        CollisionParams
    );
    // GroundBeamActor->SetBeamEndpoints(PalmPos, HitResult.Location);
...
void AHanTracking::SetUpPostProcessVolume()
{
    ...
    if (IsPluginLoaded)
    {
        ...
        FString MaterialPath = TEXT("/Game/MixedReality/PP_MR.PP_MR");
...
{
    // Correct path for the blueprint inside the plugin content folder
    FString BlueprintPath = TEXT("/Game/MixedReality/BP_MRControls.BP_MRControls_C");
    BlueprintClass = LoadObject<UClass>(nullptr, *BlueprintPath);

    if (BlueprintClass)
    {
        // Spawn the blueprint actor in the world
        FVector SpawnLocation = FVector(0.0f, 0.0f, 0.0f);
        FRotator SpawnRotation = FRotator(0.0f, 0.0f, 0.0f);
        AActor* SpawnedActor = GetWorld()->SpawnActor<AActor>(BlueprintClass, SpawnLocation, SpawnRotation);
